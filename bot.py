...
def build_filename(anime, season, episode, quality):
    ...
    if not tpl:
        return f"S{season:02}E{episode:02} @anifindX.mkv"
...
async def receive_thumb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in SET_THUMB_WAIT and is_admin(uid) and update.message.photo:
...
async def preview(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eps = episodes.find(
        {"anime": anime.upper(), "season": season, "quality": quality}
    ).sort("episode", 1)
